...
world_df |>
  ...
  labs(
    title = "Figure 4: American vs British Short 'a' Sounds",
    x = "F1 (Hz)", y = "F2 (Hz)", color = NULL
  ) +
  scale_color_manual(values = c("darkgrey", "grey4")) +
  theme_minimal() +
  theme(
    text = element_text(family = "Times"),
    plot.title = element_text(size = 14, face = "bold", vjust = 3.4, family = "Times"),
    axis.title.x = element_text(size = 12, vjust = -5, family = "Times"),
    axis.title.y = element_text(size = 12, vjust = 5, family = "Times"),
    strip.text.x = element_text(size = 12, color = "black", vjust = 0.5, family="Times"),
    strip.background = element_rect(color = NA, fill = "white"),
    legend.position = "none",
    plot.margin = unit(c(.5, .5, .5, .5), "inch")
  ) +
  facet_wrap(vars(region))
ggsave(file.path(".", "plots", "a-comparison.pdf"), width = 18, height = 12, units = "cm")
```
